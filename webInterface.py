...
course_ratings_dict = {'CSCI 153 HM-02': 9,
                       'CSCI 152 PO-01': 9,
                       'ECON 197 PZ-01': 7,
                       'MATH 189T HM-01': 4,
                       'ART 002 HM-01': 6,
...
@ app.callback(
    [
        Output('textarea-div', 'style'),
        Output('datatables-div', 'children')
    ],
    ...
    if clicks is not None:
        ...
        requirements_table = \
            dash_table.DataTable(
                ...
                data=requirements_template_df.to_dict(
                    'records'),
                editable=True,
                page_action='none',
                            style_header={
        ...
        add_col_button = \
            dbc.Button(
                'Add Requirement',
                id='adding-cols-button', color="success", className="mr-1", size="rg",
                style={
                    "marginLeft": 75,
                    "fontSize": 25
                })

        rating_instructions_modal = \
            html.Div(
                [
        ...
        requirement_instructions_modal = \
            html.Div(
                [
...
def update_output_2(clicks, courses_table, selected_course_row_ids, requirements_table):
    if clicks is not None:
        ...
        requirements_df = requirements_df.loc[(
            requirements_df != 0).any(axis=1)]
        ...
        for course in courses:
            courses_cost[course] = float(
                courses_df.loc[course, "Rating"])
        ...
        for i in range(len(requirements_df.columns.tolist())):
            ...
            if i == 0:
                ...
                curr_alternates = [x for x in curr_alternates if
                                   (str(x) != 'nan') and
                                   (str(x) != '0')]  # Note- will need to change later
        ...
        modal = html.Div(
            [
                dbc.Button("Input for HyperSchedule",
                           id="open-scroll", className="mr-1",
                           style={
                               "fontSize": 30}),
                ...
                dbc.Modal(
                    [
                        dbc.ModalHeader(
                            "Copy this and enter it into HyperSchedule"),
                        dbc.ModalBody(jsonOutput),
                        dbc.ModalFooter(
                            dbc.Button("Close", id="close-scroll",
                                       className="ml-auto",
                                       style={
                                           "fontSize": 30
                                       })
        ...
        list_group = dbc.ListGroup(
            [
                dbc.ListGroupItem(
                    f"{coursesChosenByOptimizer[i][0]}: {coursesChosenByOptimizer[i][1]}", color=colors[i % len(colors)]) for i in range(len(coursesChosenByOptimizer))
            ],
            style={
                "fontSize": 30
            }
        ...
        return [
            ...
            [
                dbc.Row(
                    html.H2("An Optimal Combination of Courses!"),
                    justify="center"
                ),
                dbc.Row(
                    list_group,
...
def update_columns(n_clicks, existing_columns):
    ...
    if n_clicks > 0:
        ...
        existing_columns.append({
            'id': f"Req/Constraint {i+1}", 'name': f"Req {i}",
